...
Open [`docs/preview/index.html`](docs/preview/index.html) in a browser
to click through every browser-facing page (login, logout, requester,
approver, confirmer) and every MCP `*_list` tool's text output, in
both light and dark themes.

The preview site is regenerated by `npm run preview` from a single
source of truth in [`scripts/preview/`](scripts/preview/). After
changing a template or formatter, run `npm run preview` and commit the
regenerated `docs/preview/` files alongside the source change. The
`npm run preview:check` gate (part of `npm run check` and CI) refuses
any drift and tells you which surface is missing or out of date — see
[ADR-0012](docs/adr/0012-preview-site-and-list-scenarios.md) and the
