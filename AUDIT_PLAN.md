...
Privileged Identity Management. v0.1.0 candidate. 24 tools across
auth + three PIM surfaces (group, role-entra, role-azure). Auth is
MSAL via interactive browser loopback, with one login covering both
Microsoft Graph and Azure Resource Manager. Single maintainer
(`@simongottschlag`).
...
| D-2   | M   | S      | `AGENTS.md` says "Phase 5 in progress" but `CHANGELOG.md` records `[0.1.0] - 2026-05-08` as shipped. README also self-describes as "v0.1.0 candidate". Pick one truth — when 0.1.0 is published, update AGENTS.md and the README banner.                                                                                                                                                                                                       |
...
- **D-2** Update `AGENTS.md` to match shipped status (drop "Phase 5 in progress"). Sync the README banner.
