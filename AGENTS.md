...
**Pre-release.** All four phases of the roadmap (foundation, group, Entra-role, Azure-role surfaces) are complete. The MCP server exposes the full **24 tools**: 3 auth + 7 group + 7 Entra-role + 7 Azure-role. No version has been tagged yet. The first release will be cut once the remaining pre-release polish lands. See `.tmp/roadmap.md` and `.tmp/progress.md` for historical context.
