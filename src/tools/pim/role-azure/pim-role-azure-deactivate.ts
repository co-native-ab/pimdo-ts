...
// PIM Azure-role assignments via the confirmer browser flow.
...
const ItemSchema = z.object({
  instanceId: z
    .string()
    .min(1)
    .describe("The active assignment instance id from pim_role_azure_active_list."),
...
function pickInstances(
  all: readonly RoleAzureActiveAssignment[],
  items: { instanceId: string; reason?: string }[] | undefined,
): {
  selected: { instance: RoleAzureActiveAssignment; reason?: string }[];
  missing: string[];
} {
  ...
  const selected: { instance: RoleAzureActiveAssignment; reason?: string }[] = [];
...
function handler(config: ServerConfig): ToolCallback<typeof inputSchema> {
  return async (args, { signal }) => {
    try {
      const active = await listActiveRoleAzureAssignments(config.armClient, signal);
      ...
      if (selected.length === 0) {
        const reason =
          missing.length > 0
            ...
            : "No active PIM Azure-role assignments to deactivate.";
      ...
      const rows: ConfirmerRowSpec[] = selected.map(
        (s): ConfirmerRowSpec => ({
          id: s.instance.id,
          label: roleLabel(
            s.instance.properties.roleDefinitionId,
            s.instance.properties.expandedProperties,
          ),
          subtitle: s.instance.properties.endDateTime
            ? `${scopeLabel(s.instance.properties.scope, s.instance.properties.expandedProperties)} — Active until ${s.instance.properties.endDateTime}`
            : scopeLabel(s.instance.properties.scope, s.instance.properties.expandedProperties),
      ...
      const handle = await runConfirmerFlow(
        {
          heading: "Deactivate PIM Azure-role assignments",
      ...
      for (const row of result.rows) {
        ...
        const scope = scopeFromAssignment(instance);
        if (!scope) {
          errors.push(`instance ${row.id} has no resolvable ARM scope`);
          continue;
        }
        try {
          const created = await requestRoleAzureDeactivation(
            config.armClient,
            scope,
            {
              principalId,
          ...
          const label = roleLabel(
            instance.properties.roleDefinitionId,
            instance.properties.expandedProperties,
          );
      ...
      const head =
        summaries.length > 0
          ? `Submitted ${String(summaries.length)} PIM Azure-role deactivation request(s):\n${summaries.join("\n")}`
...
export const pimRoleAzureDeactivateTool: Tool<typeof inputSchema> = {
  def,
  inputSchema,
