...
//
// Library entry: exports the composition root (`createMcpServer`),
// shared startup helpers (`validateApiBaseUrl`, `resolveStaticToken`,
// `wireShutdownSignals`), and constants (`VERSION`, `CLIENT_ID`).
// The executable entry point lives in `./main.ts`, which builds to the
// `dist/index.js` binary referenced by `package.json#bin` and the MCPB
// bundle.
